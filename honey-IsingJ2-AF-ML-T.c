...
int rand_int(int a, int b); /* a to b */
int mod (int a, int b);
...
int Nsites=2*N1*N2;
int Nprom=400;
int Neq=400;// number of montecarlosteps / sites that i wait in each temperature to thermalize
		...
		if(abs(*m/Nsites)<1.1){
			...
			*(m2p+ii)+=pow(*m,2);
			...
			for(int i=0;i<N1;i++){ 
				int j=mod(-2*i,N2); 
				for(int k=0;k<N2;k++){ // repito N2 veces 
				if (abs(*(p+mod(j,N2)+i*N2))>1 || abs(*(q+ mod(j,N2) +i*N2))>1 ){
					printf("%d %d \n ",*(p+mod(j,N2)+i*N2),*(q+ mod(j,N2) +i*N2) );
					printf("%d %d \n ",i,mod(j,N2));}
					j=mod(j+1,N2);
				}
			}
...
void montecarlo_step(int N1, int N2, double J,double J2,double T,int *p,int *q,double* e,double* m){
int rand_i=rand_int(0,N1-1);//random cell
int rand_j=rand_int(0,N2-1);
//printf("%d, %d\n",rand_i, rand_j);
